package com.android.bluetooth.library.communication;
...
import com.android.bluetooth.library.util.LogUtil;
...
public class BluetoothComunication extends Thread {
	...
	public void run() {
		 ...
		 try {
			 ...
			 sendHandler(Constants.MSG_TOAST, context.getString(R.string.connected_sucessfully));
			 ...
			 while (run) {
				 if(dataInputStream.available() > 0){
					 ...
					 sendHandler(Constants.MSG_BLUETOOTH, nameBluetooth + ": " + new String(msg));
				 }
			 }
		 }catch (IOException e) {
			 ...
			 stopComunication();
			 sendHandler(Constants.MSG_TOAST, context.getString(R.string.lost_connection));
		 }
	}
	...
	public boolean sendMessageByBluetooth(String msg){
		try {
			if(dataOutputStream != null){
				...
				sendHandler(Constants.MSG_TOAST, context.getString(R.string.no_connection));
				return false;
			}
		} catch (IOException e) {
			LogUtil.e(e.getMessage());
			
			sendHandler(Constants.MSG_TOAST, context.getString(R.string.failed_to_send_message));
			return false;
		}
	}
